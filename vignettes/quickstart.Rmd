...
ggplot(results, aes(x = x, y = y, color = eval.id)) + geom_line()
```
...
require(ggplot2)
ggplot(results, aes(x = x, y = y, color = factor(eval.n))) + geom_line() + 
  facet_grid(eval.SS ~ eval.So)  
```
```{r filter-data, results="hide"}  
require(dplyr)
filter(results, eval.n == 0.045, eval.So == 0.0015, eval.SS == 2)
```
```{r first-five, echo=FALSE}
kable(head(filter(results, eval.n==0.045, eval.So==0.0015, eval.SS==2)))
```
